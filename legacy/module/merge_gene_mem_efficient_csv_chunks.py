...
def merge_barcodes(spatial_df, expression_file, ensembl_file, output_dir, cutoff=500, chunk_size=1000, window=100):
    """Process large expression files in chunks to conserve memory."""
    os.makedirs(output_dir, exist_ok=True)
    
    # # Load spatial and ensembl data (small files)
    ...
    reader = pd.read_csv(expression_file, index_col=False, chunksize=chunk_size)
    
    # First pass: Build barcode index and expression summary
    barcode_stats = {}
    for chunk in reader:
        for barcode in chunk.columns[:]:  # Skip gene_id column
            current_count = (chunk[barcode] > 0).sum()
...
def process_single_barcode(barcode, expression_file, ensembl, output_dir, chunk_size=1000):
    """Process a single barcode's expression data."""
    expr = None
    for chunk in pd.read_csv(expression_file, index_col=False, chunksize=chunk_size):
        if barcode in chunk.columns:
            if expr is None:
                expr = chunk[[barcode]].copy()
            else:
                expr = pd.concat([expr, chunk[[barcode]]])
...
def process_barcode_group(barcodes, expression_stats, expression_file, ensembl, output_dir, cutoff, chunk_size=1000):
...
def main():
    ...
    parser.add_argument("--spatial", required=True, help="Spatial coordinate of barcodes, csv format")
    parser.add_argument("--expression", required=True, help="Spatial gene expression csv file. No geneID. Columns are barcodes.")
    ...
    parser.add_argument("--chunk_size", required=False, type=int, default=1000, help="Chunk-size pandas read csv file. (Default: 1000))")
